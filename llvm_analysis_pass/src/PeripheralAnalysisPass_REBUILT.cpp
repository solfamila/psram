...
static cl::opt<std::string> OutputFile("o", cl::desc("Output file for analysis results"), cl::value_desc("filename"));
static cl::opt<bool> VerboseOutput("v", cl::desc("Enable verbose output"));
static cl::opt<bool> ChronologicalOutput("chronological", cl::desc("Enable chronological execution order tracking"));
    ...
    if (VerboseOutput) {
        ...
        for (const auto &p : peripherals) {
            errs() << "   " << p.first << ": " << p.second.registers.size() 
                   << " registers at base 0x" << std::hex << p.second.baseAddress << std::dec << "\n";
        }
    }
}
...
void PeripheralAnalysisPass::generateAnalysisReport() {
    std::string filename = OutputFile.empty() ? "peripheral_analysis_rebuilt.json" : OutputFile;
    ...
    for (const auto &group : peripheral_groups) {
        ...
        if (peripheral_it != peripherals.end()) {
            outFile << "      \"base_address\": \"0x" << std::hex << peripheral_it->second.baseAddress << std::dec << "\",\n";
        }
        ...
        for (const auto &access : group.second) {
            ...
            outFile << "          \"address\": \"0x" << std::hex << access.address << std::dec << "\",\n";
...
static RegisterStandardPasses RegisterMyPass(PassManagerBuilder::EP_EarlyAsPossible,
                                           registerPeripheralAnalysisPass);
